...
- **Nav Mesh & Walkable Game Objects:** NavMesh stands for Navigation Mesh, which is a type of data structure used for pathfinding in games. A NavMesh is a simplified representation of the game world that is used to determine where characters can and cannot move. It is created by baking (precomputing) the level geometry into a mesh that represents walkable surfaces, and can be used by a variety of different AI systems to navigate around the level. We went into the navigation panel (Window => AI) and clicked on "bake" to generate the `NavMesh`. Before doing this, we went to the "Terrain" object in the mesh and marked it as "walkable." Others (trees, stones, rocks, etc.) were not marked as "Walkable" in the Engine. Now, Unity generates a `NavMesh` for us.
  <br><br>

- **Nav Mesh Agent:** A NavMeshAgent is a component in Unity that can be attached to a game object to enable it to navigate around a NavMesh. It has properties such as speed, acceleration, and turning speed that can be configured to control how the agent moves. The agent can be told to move to a specific location on the NavMesh, or to follow a path generated by a pathfinding algorithm. Any object having Nav Mesh Agent attached to it as a component (our enemy has it) will be able to walk on the walkable area on our Nav Mesh. Nav Mesh agents have a rigid body attached with them by default. They also have a collider attached with them.

- **Nav Mesh Hit:** A NavMeshHit is a data structure that is used to store information about a hit with a NavMesh. It is typically used in conjunction with the NavMesh.SamplePosition() function, which takes a position in the game world and returns the closest point on the NavMesh to that position. The NavMeshHit structure contains information such as the position of the hit, the normal of the surface that was hit, and the distance from the original position to the hit point. It can be used to determine if a character is standing on the ground or if it has collided with an obstacle.
  <br><br><br>
...
- **Artificial Intelligence (AI) for Path Finding:** The `UnityEngine.AI` namespace in Unity does contain classes and functions related to artificial intelligence, specifically for navigation and pathfinding. The namespace includes classes such as `NavMesh`, `NavMeshAgent`, `NavMeshObstacle`, and `NavMeshPath`, which provide functionality for creating and navigating on a NavMesh.
  <br><br>

- **Moving Nav Mesh Agent (Enemy) on the NavMesh (Terrain):** We set a random destination for our enemy (agent) using the folloing code:
...
- **Attack Points:** Attack point is a game object attached at the end of the axe's tip. This is done so that we can detect when the axe attacked the other enemy.
